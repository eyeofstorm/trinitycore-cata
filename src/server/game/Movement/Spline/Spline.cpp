...
/*  classic view:
inline void C_Evaluate(Vector3 const* vertice, float t, float const (&matrix)[4][4], Vector3 &position)
{
...
void SplineBase::init_spline(Vector3 const* controls, index_type count, EvaluationMode m, float orientation)
{
    m_mode = m;
    cyclic = false;
...
void SplineBase::init_cyclic_spline(Vector3 const* controls, index_type count, EvaluationMode m, index_type cyclic_point, float orientation)
{
    m_mode = m;
    cyclic = true;
    initialOrientation = orientation;
...
void SplineBase::InitCatmullRom(Vector3 const* controls, index_type count, index_type cyclic_point)
{
    ...
    if (cyclic)
    {
        if (cyclic_point == 0)
            ...
            points[0] = controls[0] - G3D::Vector3{ cosl(initialOrientation), sinl(initialOrientation), 0.0f };
