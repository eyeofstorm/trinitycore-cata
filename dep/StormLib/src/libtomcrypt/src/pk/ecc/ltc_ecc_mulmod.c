...
int ltc_ecc_mulmod(void *k, ecc_point *G, ecc_point *R, void *modulus, int map)
{
  ...
  for (i = 0; i < 8; i++) {
   ...
   /* M[0] == 8G */
   if ((err = ltm_desc.ecc_ptdbl(tG, M[0], modulus, mp)) != CRYPT_OK)                 { goto done; }
   if ((err = ltm_desc.ecc_ptdbl(M[0], M[0], modulus, mp)) != CRYPT_OK)               { goto done; }
   if ((err = ltm_desc.ecc_ptdbl(M[0], M[0], modulus, mp)) != CRYPT_OK)               { goto done; }

   /* now find (8+k)G for k=1..7 */
   for (j = 9; j < 16; j++) {
       if ((err = ltm_desc.ecc_ptadd(M[j-9], tG, M[j-8], modulus, mp)) != CRYPT_OK)   { goto done; }
   }
   ...
   for (;;) {
     ...
     if (--bitcnt == 0) {
       ...
       bitcnt = (int) ltm_desc.bits_per_digit;
       --digidx;
     }

     /* grab the next msb from the ltiplicand */
     i = (buf >> (ltm_desc.bits_per_digit - 1)) & 1;
     buf <<= 1;
     ...
     if (mode == 1 && i == 0) {
        if ((err = ltm_desc.ecc_ptdbl(R, R, modulus, mp)) != CRYPT_OK)                 { goto done; }
        continue;
     }
     ...
     if (bitcpy == WINSIZE) {
       ...
       if (first == 1) {
         ...
         if ((err = ltm_desc.ecc_ptadd(R, M[bitbuf-8], R, modulus, mp)) != CRYPT_OK)   { goto done; }
       }
     ...
     for (j = 0; j < bitcpy; j++) {
       ...
       if ((bitbuf & (1 << WINSIZE)) != 0) {
         if (first == 1){
            ...
            /* then add */
            if ((err = ltm_desc.ecc_ptadd(R, tG, R, modulus, mp)) != CRYPT_OK)        { goto done; }
         }
       }
     }
   }
